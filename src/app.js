...
document.addEventListener('keydown',function(e){if(e.key==='Alt'){e.preventDefault();if(selEle&&selEle.isNode()){altHeld=true;document.getElementById('altHint').style.display='block';document.getElementById('cy').style.cursor='crosshair';clrHL()}}if(e.target.tagName==='INPUT'||e.target.tagName==='TEXTAREA'||e.target.tagName==='SELECT')return;if(e.ctrlKey&&e.key==='d'){e.preventDefault();addNodeFree()}if(e.ctrlKey&&e.key==='s'){e.preventDefault();void doSave()}if(e.ctrlKey&&e.key==='z'){e.preventDefault();if(e.shiftKey)redo();else undo()}if(e.key==='Delete')delSel();if(e.key==='Escape'){clrSel();document.getElementById('searchBox').value='';doSearch('');closeSettings();closeExport();if(typeof closeZoteroImport==='function')closeZoteroImport();if(typeof closeReport==='function')closeReport()}});
...
function posAB(){var btn=document.getElementById('addBtn');if(!selEle||!selEle.isNode()){btn.style.display='none';return}var pos=selEle.renderedPosition();btn.style.left=(pos.x+24)+'px';btn.style.top=(pos.y-14)+'px';btn.style.display='block'}
...
if(window.mentegraf){window.mentegraf.onMenuAction(function(a){if(a==='add-node')addNodeFree();if(a==='delete')delSel();if(a==='relayout')doLayout();if(a==='fit')cy.fit(null,50);if(a==='save')void doSave();if(a==='new'){initConfig();init();clrSel()}if(a==='undo')undo();if(a==='redo')redo()});window.mentegraf.onFileOpened(function(data){loadProject(data)})}
